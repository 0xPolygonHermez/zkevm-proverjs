...
function generate(index, offset, len, config) {
    ...
    const legend512 = rule.slice(0, (Number(offset) * 2)) + '#'.repeat(Number(_len) * 2) + ((offset + _len) >= 64n ? '' : rule.slice(-2 * Number(64n - offset - _len)));
    ...
    // read operations
    let data = {config, a, b, l1: legend512.slice(0, 64), l2: legend512.slice(64), wr: false, d: 0n, e: 0n};
    dump({...data, index: index, op: v, c0: _c});
    dump({...data, index: index + 1, op: vle, c0: _c + ` + ${cpre}LITTLE_ENDIAN`});
    dump({...data, index: index + 2, op: v << (32n - _len) * 8n, c0: _c  + ` + ${cpre}LEFT_ALIGN`});
    dump({...data, index: index + 3, op: vle << (32n - _len) * 8n, c0: _c + ` + ${cpre}LEFT_ALIGN + ${cpre}LITTLE_ENDIAN`});
    ...
    dump({...data, index: index + 5, d, e, c0: _c + ` + ${cpre}LITTLE_ENDIAN`});
    [d, e] = split512to256(W | ((config.V >> shrV) << shlV))
    dump({...data, index: index + 6, d, e, c0: _c + ` + ${cpre}LEFT_ALIGN`});
    [d, e] = split512to256(W | (toLE(config.V >> shrV, _len) << shlV));
    dump({...data, index: index + 7, d, e, c0: _c + ` + ${cpre}LEFT_ALIGN + ${cpre}LITTLE_ENDIAN`});
    
    return index + 8;
}

if (!argv.js && !argv.zkasm) {
    console.log('usage: node gen.js [--zkasm|--js]')
} else {
    let index = 0;
    for (let offset = 0n; offset < 65n; ++offset) {
        for (let len = 0n; len < 33n; ++len) {
            index = generate(index, offset, len, {M, V, output: argv.js ? 'js':'zkasm', cpre: argv.js ? '':'%MA_'});
        }
    }
}
