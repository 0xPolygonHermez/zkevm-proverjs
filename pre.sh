...
checkAllMandatoryOptArgs() {
    ...
    checkMandatoryOptArg pfsetup $npm_config_pfsetup pilfflonksetup
...
usage() {
    ...
    echo " --pfsetup=<builder>               alternative binary to generate constanttree (ex: ../pilfflonk/build/pfSetup)"
...
PFSETUP="${npm_config_pfsetup:=$PILSTARK/fflonk/main_setup.js -p $BDIR/final.pil -f $BDIR/final.fflonkinfo.json -t build/powersOfTau28_hez_final.ptau -c $BDIR/final.const -z $BDIR/final.zkey -e 5}"
# [ ! -z $npm_config_nth ] &&
NTH="${npm_config_nth}"
true
