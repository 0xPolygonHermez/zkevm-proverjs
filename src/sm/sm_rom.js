...
module.exports.buildConstants = async function buildConstants(pols, rom) {
    ...
    for (let i=0; i<N; i++) {
        ...
        /*
            code generated with:
            node tools/pil_pol_table/bits_compose.js "arithEq0,arithEq1,arithEq2,assert,bin,hashK,hashKDigest,hashKLen,hashP,hashPDigest,hashPLen,hashP_fe12,ind,indRR,isMem,isStack,JMP,JMPC,JMPN,memAlignRD,memAlignWR,memAlignWR8,mOp,mWR,repeat,setA,setB,setC,setCTX,setD,setE,setGAS,setHASHPOS,setPC,setRCX,setRR,setSP,setSR,sRD,sWR,useCTX,useJmpAddr,JMPZ,call,return,hashK1,hashP1,useElseAddr" -B -e -p "rom.program[pIndex]."
        */
        ...
        + (rom.program[pIndex].hashP_fe12 ? (2n**11n * BigInt(rom.program[pIndex].hashP_fe12)) : 0n)
        + (rom.program[pIndex].ind ? (2n**12n * BigInt(rom.program[pIndex].ind)) : 0n)
        + (rom.program[pIndex].indRR ? (2n**13n * BigInt(rom.program[pIndex].indRR)) : 0n)
        + (rom.program[pIndex].isMem ? (2n**14n * BigInt(rom.program[pIndex].isMem)) : 0n)
        + (rom.program[pIndex].isStack ? (2n**15n * BigInt(rom.program[pIndex].isStack)) : 0n)
        + (rom.program[pIndex].JMP ? (2n**16n * BigInt(rom.program[pIndex].JMP)) : 0n)
        + (rom.program[pIndex].JMPC ? (2n**17n * BigInt(rom.program[pIndex].JMPC)) : 0n)
        + (rom.program[pIndex].JMPN ? (2n**18n * BigInt(rom.program[pIndex].JMPN)) : 0n)
        + (rom.program[pIndex].memAlignRD ? (2n**19n * BigInt(rom.program[pIndex].memAlignRD)) : 0n)
        + (rom.program[pIndex].memAlignWR ? (2n**20n * BigInt(rom.program[pIndex].memAlignWR)) : 0n)
        + (rom.program[pIndex].memAlignWR8 ? (2n**21n * BigInt(rom.program[pIndex].memAlignWR8)) : 0n)
        + (rom.program[pIndex].mOp ? (2n**22n * BigInt(rom.program[pIndex].mOp)) : 0n)
        + (rom.program[pIndex].mWR ? (2n**23n * BigInt(rom.program[pIndex].mWR)) : 0n)
        + (rom.program[pIndex].repeat ? (2n**24n * BigInt(rom.program[pIndex].repeat)) : 0n)
        + (rom.program[pIndex].setA ? (2n**25n * BigInt(rom.program[pIndex].setA)) : 0n)
        + (rom.program[pIndex].setB ? (2n**26n * BigInt(rom.program[pIndex].setB)) : 0n)
        + (rom.program[pIndex].setC ? (2n**27n * BigInt(rom.program[pIndex].setC)) : 0n)
        + (rom.program[pIndex].setCTX ? (2n**28n * BigInt(rom.program[pIndex].setCTX)) : 0n)
        + (rom.program[pIndex].setD ? (2n**29n * BigInt(rom.program[pIndex].setD)) : 0n)
        + (rom.program[pIndex].setE ? (2n**30n * BigInt(rom.program[pIndex].setE)) : 0n)
        + (rom.program[pIndex].setGAS ? (2n**31n * BigInt(rom.program[pIndex].setGAS)) : 0n)
        + (rom.program[pIndex].setHASHPOS ? (2n**32n * BigInt(rom.program[pIndex].setHASHPOS)) : 0n)
