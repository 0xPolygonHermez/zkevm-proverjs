...
class FullTracer {
    ...
    onUpdateStorage(ctx, params) {
        ...
        if (this.full_trace.length > 0) {
            const singleCallTrace = this.full_trace[this.full_trace.length - 1];
    ...
    onFinishTx(ctx) {
        ...
        if (this.full_trace.length) {
            ...
            this.finalTrace.responses[this.finalTrace.responses.length - 1].full_trace.steps = this.full_trace;
            if (this.finalTrace.responses[this.finalTrace.responses.length - 1].error === '') {
                this.finalTrace.responses[this.finalTrace.responses.length - 1].error = lastOpcodeCall.error;
            }
        }
    ...
    onFinishBatch(ctx) {
        ...
        this.finalTrace.responses.forEach((r) => {
            if (typeof r.full_trace !== 'undefined') {
                r.full_trace.context.batch = this.finalTrace.new_acc_input_hash;
            }
    ...
    onOpcode(ctx, params) {
        ...
        if (prevStep && opIncContext.includes(prevStep.opcode) && prevStep.depth !== singleInfo.depth) {
            ...
            singleInfo.contract.gas = this.txGAS[this.depth];
            singleCallTrace.contract.gas = this.txGAS[this.depth];
