...
const { SMT } = require('@0xpolygonhermez/zkevm-commonjs');
...
module.exports = async function execute(pols, input, rom, config = {}, metadata = {}) {
    ...
    let op7; let op6; let op5; let op4; let op3; let op2; let op1; let
        op0;
    ...
    try {
        for (let step = 0; step < stepsN; step++) {
            ...
            if (l.inA) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inA), ctx.A[0])),
            ...
            if (l.inB) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inB), ctx.B[0])),
            ...
            if (l.inC) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inC), ctx.C[0])),
            ...
            if (l.inD) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inD), ctx.D[0])),
            ...
            if (l.inE) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inE), ctx.E[0])),
            ...
            if (l.inSR) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inSR), ctx.SR[0])),
            ...
            if (l.inROTL_C) {
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(op0, Fr.mul(Fr.e(l.inROTL_C), ctx.C[7])),
            ...
            if (l.inFREE) {
                ...
                if (l.freeInTag.op == '') {
                    ...
                    if (l.sWR == 1) {
                        ...
                        if (fullTracer) {
                            ...
                            if (fullTracer.options.verbose.enable) { fullTracer.onAccessed(ctx.Fr, ctx.A, ctx.C, ctx.B); }
                        }
                    ...
                    if (l.hashK || l.hashK1) {
                        if (typeof ctx.hashK[addr] === 'undefined') {
                            ctx.hashK[addr] = {
                                data: [], reads: {}, digestCalled: false, lenCalled: false, sourceRef,
                            };
                        }
                ...
                [op0, op1, op2, op3, op4, op5, op6, op7] = [Fr.add(Fr.mul(Fr.e(l.inFREE), fi[0]), op0),
            ...
            if (l.hashPDigest || l.sWR) {
                ...
                required.Binary.push({
                    a: op, b: 0n, c: op, opcode: 1, type: 2,
                });
            }
            ...
            if (l.arithEq0 || l.arithEq1 || l.arithEq2 || l.arithEq3 || l.arithEq4 || l.arithEq5) {
                if (l.arithEq0 && !l.arithEq1 && !l.arithEq2 && !l.arithEq3 && !l.arithEq4 && !l.arithEq5) {
                    ...
                    if (!Scalar.eq(Scalar.add(Scalar.mul(A, B), C), Scalar.add(Scalar.shl(D, 256), op))) {
                        ...
                        const left = Scalar.add(Scalar.mul(A, B), C);
                        const right = Scalar.add(Scalar.shl(D, 256), op);
                    ...
                    required.Arith.push({
                        ...
                        y2: ctx.D,
                        x3: Fr8zero,
                        y3: [op0, op1, op2, op3, op4, op5, op6, op7],
                        selEq0: 1,
                        selEq1: 0,
                        selEq2: 0,
                        selEq3: 0,
            ...
            if (l.memAlignRD || l.memAlignWR || l.memAlignWR8) {
                ...
                if (!l.memAlignRD && l.memAlignWR && !l.memAlignWR8) {
                    ...
                    const _W1 = Scalar.bor(
                        Scalar.band(m1, MASK_256 >> (offset * 8n)),
                    ...
                    required.MemAlign.push({
                        m0, m1, v, w0, w1, offset, wr256: 1n, wr8: 0n,
                    });
                } else if (!l.memAlignRD && !l.memAlignWR && l.memAlignWR8) {
                    const w0 = safeFea2scalar(Fr, ctx.D);
                    const _W0 = Scalar.bor(
                        Scalar.band(m0, Scalar.shr(byteMaskOn256, 8n * offset)),
                        Scalar.shl(Scalar.band(v, 0xFF), 8n * (31n - offset)),
                    );
                    ...
                    required.MemAlign.push({
                        m0, m1: 0n, v, w0, w1: 0n, offset, wr256: 0n, wr8: 1n,
                    });
                } else if (l.memAlignRD && !l.memAlignWR && !l.memAlignWR8) {
                    const m1 = safeFea2scalar(Fr, ctx.B);
                    const leftV = Scalar.band(Scalar.shl(m0, offset * 8n), MASK_256);
                    ...
                    pols.memAlignWR8[i] = 0n;
                    required.MemAlign.push({
                        m0, m1, v, w0: Fr.zero, w1: Fr.zero, offset, wr256: 0n, wr8: 0n,
                    });
                } else {
...
function evalCommand(ctx, tag) {
    ...
    } if (tag.op == 'or' || tag.op == 'and' || tag.op == 'gt' || tag.op == 'ge' || tag.op == 'lt' || tag.op == 'le'
               || tag.op == 'eq' || tag.op == 'ne' || tag.op == 'not') {
        return eval_logical_operation(ctx, tag);
    } if (tag.op == 'bitand' || tag.op == 'bitor' || tag.op == 'bitxor' || tag.op == 'bitnot' || tag.op == 'shl' || tag.op == 'shr') {
        return eval_bit_operation(ctx, tag);
    } if (tag.op == 'if') {
        return eval_if(ctx, tag);
    } if (tag.op == 'getMemValue') {
...
function eval_bit_operation(ctx, tag) {
    ...
    default: throw new Error(`bit operation ${tag.op} not defined ${ctx.sourceRef}`);
    }
}
...
function eval_logical_operation(ctx, tag) {
    ...
    default: throw new Error(`logical operation ${tag.op} not defined ${ctx.sourceRef}`);
    }
}
...
function eval_getTxsLen(ctx, tag) {
    ...
    return [
        ctx.Fr.e((ctx.input.batchL2Data.length - 2) / 2),
        ctx.Fr.zero,
        ctx.Fr.zero,
        ctx.Fr.zero,
        ctx.Fr.zero,
...
function eval_log(ctx, tag) {
    ...
    if (typeof (frLog) === 'number') {
        ...
        if (label !== 'notset') { console.log('       Label: ', label); }
...
function eval_memAlignWR_W0(ctx, tag) {
    ...
    return scalar2fea(ctx.Fr, Scalar.bor(
        Scalar.band(m0, Scalar.shl(Mask256, (32n - offset) * 8n)),
...
function eval_memAlignWR_W1(ctx, tag) {
    ...
    return scalar2fea(ctx.Fr, Scalar.bor(
        Scalar.band(m1, Scalar.shr(Mask256, offset * 8n)),
...
function eval_memAlignWR8_W0(ctx, tag) {
    ...
    return scalar2fea(ctx.Fr, Scalar.bor(
        Scalar.band(m0, Scalar.sub(Mask256, Scalar.shl(0xFFn, bits))),
