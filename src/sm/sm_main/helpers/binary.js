...
/*
    ...
    #calculate(operation, a, b) {
        ...
        switch (operation) {
            case ADD:
                return [Scalar.band(Scalar.add(a, b), MASK_256_BITS), 
                        (((a + b) >> 256n) > 0n) ? 1n : 0n];
        
            case SUB:
                return  [Scalar.band(Scalar.add(Scalar.sub(a, b), P2_256), MASK_256_BITS),
                         ((a - b) < 0n) ? 1n : 0n];
            case LT:
                return [Scalar.lt(a, b) ? 1n : 0n];

            case SLT: {                
                const signedA = Scalar.geq(a, P2_255) ? Scalar.sub(a, P2_256): a;
                const signedB = Scalar.geq(b, P2_255) ? Scalar.sub(b, P2_256): b;
                return [Scalar.lt(signedA, signedB) ? 1n : 0n];
            }

            case EQ:
                return [Scalar.eq(a, b) ? 1n : 0n];

            case AND: {
                const c = Scalar.band(a, b);
                return [c, Scalar.eq(c, 0n) ? 0n:1n];
            }

            case OR: 
                return [Scalar.bor(a, b), 0n];
 
            case XOR:
                return [Scalar.bxor(a, b), 0n];
