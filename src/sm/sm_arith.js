...
function buildByte2Bits16(pols, N) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        pols.SEL_BYTE2_BIT19.push(i < modB1 ? 0n:1n);
        pols.BYTE2_BIT19.push(BigInt(value));
    }
}
...
function buildClocks(pols, N, clocksByCycle) {
    for (let i = 0; i < clocksByCycle; i++) {
        for (let j = 0; j < N; ++j) {
            pols.CLK[i].push(((j + (clocksByCycle - i)) % clocksByCycle) == 0 ? 1n : 0n);
        }
    }
}
...
function buildBitsRange(pols, N, name, bits) {
    ...
    for (let i = 0; i < N; i++) {
        pols[name].push(BigInt(i % moduleBase));
    }
}
